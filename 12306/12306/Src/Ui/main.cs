...
namespace _12306
{
    public partial class main : Form
    {
        ...
        private string GetYZM()
        {
            ...
            Stream stream = null;
            stream = HttpUtils.GetResponseImage(url);
            ...
            if (this.IsHandleCreated)
            {
                this.BeginInvoke((ThreadStart)delegate()
                {
                    if (txtCFD.Tag != null && txtMDD.Tag != null)
                    {
                        ...
                        this.BeginInvoke((ThreadStart)delegate()
                        {
                            string checiJson = HttpUtils.GetResponse(url, "POST", p);
                            if (checiJson != "")
                            {
                                myCheci = JsonUtils.DeserializeToObj<List<Checi>>(checiJson);
        ...
        {
            foreach (Cookie cookie in HttpUtils.CookieContainers.GetCookies(new Uri("https://dynamic.12306.cn/otsweb/loginAction.do?method=login")))
            {
            ...
            if (dgv.Columns[e.ColumnIndex].Name == "colgoupiao")
            {
                ...
                if (cell1.Value.ToString() == "预定")
                {
                    ...
                    this.BeginInvoke((ThreadStart)delegate()
                    {
                        ...
                        string str = HttpUtils.GetResponse(url, "POST", data);
                        if (!str.Contains("目前您还有未处理的订单"))
                        {
                            ...
                            data = string.Empty;
                            string html = HttpUtils.GetResponse(url, "POST", data);
                            string temp1 = html;
        ...
        private void LoadPasssenger()
        {
            ...
            Bw.DoWork += new DoWorkEventHandler(delegate(object sender, DoWorkEventArgs e)
            {
                ...
                pList = JsonUtils.DeserializeToObj<PassengerList>(content);
                int controlX = 6;
                int controlY = 6;
                int i = 0;
        ...
        public void Buy() // 购票
        {
            ...
            html = HttpUtils.GetResponse(url, "POST", tjdata.Substring(0, tjdata.Length - 9), refer);
            ...
            while (true)
            {
                ...
                html = HttpUtils.GetResponse(url, "Post", "");
        ...
        public string Order(string date, string trainno, string traincode, string fromno, string from, string tono, string to, string starttime, string endtime, string passenger) // 提交订单
        {
            ...
            data_buy += "&tFlag=dc";
            tjdata = data_buy;
            string html = HttpUtils.GetResponse(url, "POST", data_buy);
            return html;
        }
